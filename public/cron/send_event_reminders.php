...
WHERE r.status='confirmed' AND e.event_date=DATE_ADD(CURDATE(), INTERVAL 1 DAY)
...
foreach ($db->query($sql)->fetchAll() as $row) {
    ...
    $stmt = $db->prepare("INSERT INTO email_logs (reservation_id,event_id,email,type,status,error_message,sent_at,created_at) VALUES (:rid,:eid,:email,'reminder_day_before',:status,:error,NOW(),NOW())");
